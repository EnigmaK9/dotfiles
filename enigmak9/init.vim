...
" Function to yank (copy) multiple lines to the system clipboard for Chromium,
" supporting counts (e.g., 1000<Leader>yy copies 1000 lines)
function! YankLinesToChromium()
    if !executable('xclip')
        return
    endif
    let l:count = v:count1
    let l:current = line('.')
    let l:end = l:current + l:count - 1
    let l:text = join(getline(l:current, l:end), "\n") . "\n"
    call system("xclip -i -selection clipboard", l:text)
endfunction

" Map <Leader>yy in normal mode to call the function
nnoremap <Leader>yy :call YankLinesToChromium()<CR>
...
if s:use_plugins
    if has('vim_starting')
        " Press <Tab> after colorscheme command to list available themes
        colorscheme tender
    endif
